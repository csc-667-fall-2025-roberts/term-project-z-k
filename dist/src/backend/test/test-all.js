...
async function testUserService() {
    ...
    await runner.test("Should create a new user", async () => {
        const user = await userService_1.UserService.createUser(`testuser_${Date.now()}`, "test@example.com", "hashed_password_123");
        testUserId = user.id;
    ...
    await runner.test("Should retrieve user by ID", async () => {
        const user = await userService_1.UserService.getUserById(testUserId);
    ...
    await runner.test("Should retrieve user by username", async () => {
        const user = await userService_1.UserService.getUserById(testUserId);
        const foundUser = await userService_1.UserService.getUserByUsername(user.username);
    ...
    await runner.test("Should update last login", async () => {
        await userService_1.UserService.updateLastLogin(testUserId);
        const user = await userService_1.UserService.getUserById(testUserId);
        runner.assertNotNull(user.last_login, "Last login should be set");
    });
    await runner.test("Should update user stats (win)", async () => {
        await userService_1.UserService.updateStats(testUserId, true);
        const user = await userService_1.UserService.getUserById(testUserId);
    ...
    await runner.test("Should update user stats (loss)", async () => {
        await userService_1.UserService.updateStats(testUserId, false);
        const user = await userService_1.UserService.getUserById(testUserId);
    ...
    await runner.test("Should delete user", async () => {
        await userService_1.UserService.deleteUser(testUserId);
        const user = await userService_1.UserService.getUserById(testUserId);
...
async function testRoomService() {
    ...
    const user1 = await userService_1.UserService.createUser(`host_${Date.now()}`, null, "pass1");
    const user2 = await userService_1.UserService.createUser(`player_${Date.now()}`, null, "pass2");
    const user3 = await userService_1.UserService.createUser(`player2_${Date.now()}`, null, "pass3");
    let testRoomId;
    let testRoomCode;
    ...
    await runner.test("Should create a room", async () => {
        const room = await roomService_1.RoomService.createRoom("Test Room", user1.id, 4, false);
        testRoomId = room.id;
    ...
    await runner.test("Should get room by ID", async () => {
        const room = await roomService_1.RoomService.getRoomById(testRoomId);
    ...
    await runner.test("Should get room by code", async () => {
        const room = await roomService_1.RoomService.getRoomByCode(testRoomCode);
        runner.assertNotNull(room, "Room should exist");
        runner.assertEqual(room.code, testRoomCode, "Room code should match");
    });
    await runner.test("Should add member to room", async () => {
        const member = await roomService_1.RoomService.addMember(testRoomId, user1.id);
    ...
    await runner.test("Should add second member to room", async () => {
        const member = await roomService_1.RoomService.addMember(testRoomId, user2.id);
        runner.assertNotNull(member, "Second member should be added");
    });
    await runner.test("Should get room members", async () => {
        const members = await roomService_1.RoomService.getRoomMembers(testRoomId);
        runner.assertEqual(members.length, 2, "Should have 2 members");
    });
    await runner.test("Should get room with members", async () => {
        const roomWithMembers = await roomService_1.RoomService.getRoomWithMembers(testRoomId);
    ...
    await runner.test("Should check if room is not full", async () => {
        const isFull = await roomService_1.RoomService.isRoomFull(testRoomId);
        runner.assertEqual(isFull, false, "Room should not be full");
    });
    await runner.test("Should set player ready status", async () => {
        await roomService_1.RoomService.setPlayerReady(testRoomId, user1.id, true);
        const members = await roomService_1.RoomService.getRoomMembers(testRoomId);
    ...
    await runner.test("Should assign player orders", async () => {
        await roomService_1.RoomService.assignPlayerOrders(testRoomId);
        const members = await roomService_1.RoomService.getRoomMembers(testRoomId);
    ...
    await runner.test("Should update room status", async () => {
        await roomService_1.RoomService.updateRoomStatus(testRoomId, "in_progress");
        const room = await roomService_1.RoomService.getRoomById(testRoomId);
    ...
    await runner.test("Should remove member from room", async () => {
        await roomService_1.RoomService.removeMember(testRoomId, user2.id);
        const members = await roomService_1.RoomService.getRoomMembers(testRoomId);
        runner.assertEqual(members.length, 1, "Should have 1 member left");
    });
    await runner.test("Should delete room", async () => {
        await roomService_1.RoomService.deleteRoom(testRoomId);
        const room = await roomService_1.RoomService.getRoomById(testRoomId);
        runner.assertNull(room, "Room should be deleted");
    });
    // Cleanup
    await userService_1.UserService.deleteUser(user1.id);
    await userService_1.UserService.deleteUser(user2.id);
    await userService_1.UserService.deleteUser(user3.id);
    return runner.summary();
}
async function testGameService() {
    ...
    // Setup test data
    const player1 = await userService_1.UserService.createUser(`p1_${Date.now()}`, null, "pass1");
    const player2 = await userService_1.UserService.createUser(`p2_${Date.now()}`, null, "pass2");
    const player3 = await userService_1.UserService.createUser(`p3_${Date.now()}`, null, "pass3");
    const room = await roomService_1.RoomService.createRoom("Game Test Room", player1.id);
    await roomService_1.RoomService.addMember(room.id, player1.id);
    await roomService_1.RoomService.addMember(room.id, player2.id);
    await roomService_1.RoomService.addMember(room.id, player3.id);
    ...
    await runner.test("Should create a game", async () => {
        const game = await gameService_1.GameService.createGame(room.id, [
            player1.id,
            player2.id,
            player3.id,
        ]);
    ...
    await runner.test("Top card should not be an 8", async () => {
        const game = await gameService_1.GameService.getGameById(testGameId);
    ...
    await runner.test("Should create hands for all players", async () => {
        const hand1 = await gameService_1.GameService.getHand(testGameId, player1.id);
        const hand2 = await gameService_1.GameService.getHand(testGameId, player2.id);
        const hand3 = await gameService_1.GameService.getHand(testGameId, player3.id);
    ...
    await runner.test("Should get active game by room", async () => {
        const game = await gameService_1.GameService.getActiveGameByRoom(room.id);
        runner.assertNotNull(game, "Active game should exist");
        runner.assertEqual(game.id, testGameId, "Game ID should match");
    });
    await runner.test("Should get game state", async () => {
        const gameState = await gameService_1.GameService.getGameState(testGameId);
    ...
    await runner.test("Should draw a card", async () => {
        const beforeHand = await gameService_1.GameService.getHand(testGameId, player1.id);
        const beforeCount = beforeHand.card_count;
        const drawnCard = await gameService_1.GameService.drawCard(testGameId, player1.id);
        ...
        const afterHand = await gameService_1.GameService.getHand(testGameId, player1.id);
    ...
    await runner.test("Should play a card", async () => {
        const hand = await gameService_1.GameService.getHand(testGameId, player1.id);
        const cards = JSON.parse(hand.cards);
        const cardToPlay = cards[0];
        await gameService_1.GameService.playCard(testGameId, player1.id, cardToPlay);
        const game = await gameService_1.GameService.getGameById(testGameId);
        const topCard = JSON.parse(game.top_card);
    ...
    await runner.test("Should update hand after playing card", async () => {
        ...
        await gameService_1.GameService.updateHand(testGameId, player1.id, newCards);
        const updatedHand = await gameService_1.GameService.getHand(testGameId, player1.id);
    ...
    await runner.test("Should set next player", async () => {
        await gameService_1.GameService.setNextPlayer(testGameId, player2.id);
        const game = await gameService_1.GameService.getGameById(testGameId);
    ...
    await runner.test("Should reverse direction", async () => {
        const beforeGame = await gameService_1.GameService.getGameById(testGameId);
        const beforeDirection = beforeGame.direction;
        await gameService_1.GameService.reverseDirection(testGameId);
        const afterGame = await gameService_1.GameService.getGameById(testGameId);
    ...
    await runner.test("Should get turn history", async () => {
        const history = await gameService_1.GameService.getTurnHistory(testGameId);
    ...
    await runner.test("Should finish game", async () => {
        await gameService_1.GameService.finishGame(testGameId, player1.id);
        const game = await gameService_1.GameService.getGameById(testGameId);
    ...
    try {
        ...
        const existingRoom = await roomService_1.RoomService.getRoomById(room.id);
        if (existingRoom) {
...
async function testEdgeCases() {
    ...
    await runner.test("Should handle non-existent user ID", async () => {
        const user = await userService_1.UserService.getUserById(99999);
    ...
    await runner.test("Should handle non-existent room ID", async () => {
        const room = await roomService_1.RoomService.getRoomById(99999);
    ...
    await runner.test("Should handle non-existent game ID", async () => {
        const game = await gameService_1.GameService.getGameById(99999);
    ...
    await runner.test("Should handle non-existent room code", async () => {
        const room = await roomService_1.RoomService.getRoomByCode("XXXXXX");
    ...
    await runner.test("Should handle getting hand for non-existent game", async () => {
        const hand = await gameService_1.GameService.getHand(99999, 1);
        runner.assertNull(hand, "Should return null for non-existent hand");
    });
    await runner.test("Should handle empty turn history", async () => {
        const history = await gameService_1.GameService.getTurnHistory(99999);
    ...
    await runner.test("Should prevent duplicate room members", async () => {
        const user = await userService_1.UserService.createUser(`dup_${Date.now()}`, null, "pass");
        const room = await roomService_1.RoomService.createRoom("Dup Test", user.id);
        await roomService_1.RoomService.addMember(room.id, user.id);
        await runner.assertThrows(async () => {
            await roomService_1.RoomService.addMember(room.id, user.id);
        }, "Should throw error for duplicate member");
        await roomService_1.RoomService.deleteRoom(room.id);
        await userService_1.UserService.deleteUser(user.id);
    });
    return runner.summary();
}
async function testTransactions() {
    ...
    await runner.test("Should rollback game creation on error", async () => {
        const user1 = await userService_1.UserService.createUser(`trans1_${Date.now()}`, null, "pass");
        const user2 = await userService_1.UserService.createUser(`trans2_${Date.now()}`, null, "pass");
        const room = await roomService_1.RoomService.createRoom("Trans Test", user1.id);
        ...
        try {
            ...
            await gameService_1.GameService.createGame(99999, [user1.id, user2.id]);
