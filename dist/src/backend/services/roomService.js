...
class RoomService {
    ...
    static async createRoom(name, hostId, maxPlayers = 4, isPrivate = false) {
        ...
        const result = await database_1.db.query(`INSERT INTO rooms (name, code, host_id, max_players, is_private)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [name, code, hostId, cappedMax, isPrivate]);
        return result.rows[0];
    }
    static async setPlayerReady(roomId, userId, isReady) {
        const result = await database_1.db.query(`UPDATE room_members
       SET is_ready = $1
       WHERE room_id = $2 AND user_id = $3`, [isReady, roomId, userId]);
    ...
    // - Otherwise => 'waiting'
    static async evaluateRoomStatus(roomId) {
    ...
    static async getPlayerReadyStatus(roomId, userId) {
        const result = await database_1.db.query(`SELECT is_ready FROM room_members
    ...
    static async areAllPlayersReady(roomId) {
        const result = await database_1.db.query(`SELECT COUNT(*) as total, SUM(CASE WHEN is_ready THEN 1 ELSE 0 END) as ready
       FROM room_members
    ...
    static async getRoomById(id) {
        const result = await database_1.db.query('SELECT * FROM rooms WHERE id = $1', [id]);
        return result.rows[0];
    }
    static async getRoomByCode(code) {
        const result = await database_1.db.query('SELECT * FROM rooms WHERE code = $1', [code]);
        return result.rows[0];
    }
    static async getRoomWithMembers(roomId) {
        const room = await this.getRoomById(roomId);
        if (!room)
            return undefined;
        const result = await database_1.db.query(`SELECT rm.*, u.username
    ...
    static async getAvailableRooms() {
        const result = await database_1.db.query(`SELECT r.* FROM rooms r
    ...
    static async updateRoomStatus(roomId, status) {
        await database_1.db.query('UPDATE rooms SET status = $1 WHERE id = $2', [status, roomId]);
    }
    static async deleteRoom(roomId) {
        await database_1.db.query('DELETE FROM rooms WHERE id = $1', [roomId]);
    }
    static async addMember(roomId, userId) {
        ...
        const existing = await database_1.db.query(`SELECT * FROM room_members WHERE room_id = $1 AND user_id = $2`, [roomId, userId]);
        ...
        const result = await database_1.db.query(`INSERT INTO room_members (room_id, user_id)
       VALUES ($1, $2)
       RETURNING *`, [roomId, userId]);
    ...
    static async removeMember(roomId, userId) {
        await database_1.db.query(`DELETE FROM room_members
       WHERE room_id = $1 AND user_id = $2`, [roomId, userId]);
    ...
    static async assignPlayerOrders(roomId) {
        const result = await database_1.db.query(`SELECT id FROM room_members WHERE room_id = $1 ORDER BY joined_at`, [roomId]);
        const members = result.rows;
        for (let i = 0; i < members.length; i++) {
            await database_1.db.query(`UPDATE room_members SET player_order = $1 WHERE id = $2`, [i, members[i].id]);
        }
    }
    static async getRoomMembers(roomId) {
        const result = await database_1.db.query('SELECT * FROM room_members WHERE room_id = $1', [roomId]);
        return result.rows;
    }
